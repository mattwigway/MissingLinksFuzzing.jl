    ...
    for e1 in eachrow(G.edges)
        for e2 in eachrow(G.edges)
            ...
            if geog_dist < MAX_GEOG_DIST
                ...
                len_e1 = AG.geomlength(e1.geometry)
                pos_e1 = LibGEOS.project(e1.geometry, pt_e1)
                len_e2 = AG.geomlength(e2.geometry)
                pos_e2 = LibGEOS.project(e2.geometry, pt_e2)
                ...
                if net_dist > MIN_NET_DIST
                    ...
                    if !found_this_link
                        @warn "Link from $pt_e1 @ $pos_e1 / $len_e1 to $pt_e2 @ $pos_e2 / $len_e2 with length $geog_dist not found, seed $(G.settings.seed)"
                    end
                end
            end
        end
